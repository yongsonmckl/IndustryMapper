with ranked_locations as (
  select
    ...
    row_number() over (
      ...
      order by is_canonical desc, confidence_score desc, created_at asc, id asc
    ) as location_rank
  from public.event_locations
)
update public.event_locations el
set is_canonical = ranked_locations.location_rank = 1
from ranked_locations
where ranked_locations.id = el.id;

create unique index if not exists event_locations_one_canonical_idx
